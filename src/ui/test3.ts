...
export class SidebarOverviewView extends ItemView {
  ...
  private render() {
    ...
    try {
      ...
      // 根据模式渲染不同布局
      if (this.displayMode === 'sidebar') {
        console.log('[render] Rendering sidebar mode');
        this.renderSidebarMode(container);
      } else {
        console.log('[render] Rendering main mode');
      ...
      // ✅ 关键修复：确保无论如何都重置 isRendering
      this.isRendering = false;
      console.log('[render] Set isRendering = false');
    }
  }
  ...
  private renderSidebarToolbar(container: HTMLElement) {
    ...
    filterOptions.forEach(({ mode, icon, label }) => {
      ...
      chip.addEventListener('click', () => {
        console.log('[Filter Click] Clicked', mode, 'current:', this.filterMode);
        
        if (this.filterMode !== mode) {
          console.log('[Filter Click] Changing filter mode from', this.filterMode, 'to', mode);
          
          this.filterMode = mode;
          this.shouldRestoreScroll = false;
          
          // ✅ 关键修复：确保 isRendering 为 false
          if (this.isRendering) {
            console.warn('[Filter Click] isRendering is true, forcing to false');
            this.isRendering = false;
          }
          ...
          console.log('[Filter Click] Same mode, ignoring');
        }
      });
    });
...
// });
  ...
  private renderCompactContentList(container: HTMLElement) {
    console.log('[renderCompactContentList] Start', {
      selectedFile: this.selectedFile,
      filterMode: this.filterMode
    });
    
    let units = this.getFilteredUnits();
    console.log('[renderCompactContentList] Filtered units:', units.length);
    ...
    if (this.displayMode === 'sidebar' && this.selectedFile) {
      ...
      console.log('[renderCompactContentList] After file filter:', units.length);
    }
  
    if (units.length === 0) {
      console.log('[renderCompactContentList] Empty list, showing empty state');
      ...
      if (this.filterMode !== 'all') {
        ...
        hint.addEventListener('click', () => {
          console.log('[empty-hint click] Switching to "all" mode');
          this.filterMode = 'all';
          this.shouldRestoreScroll = false;
          this.render();
        });
      ...
      console.log('[renderCompactContentList] Empty state rendered, returning');
      return; // ✅ 这里 return 是安全的，因为已经完成了渲染
    }
  
    console.log('[renderCompactContentList] Grouping units');
    const grouped = this.groupUnits(units);
    console.log('[renderCompactContentList] Grouped into', grouped.length, 'groups');
    ...
    console.log('[renderCompactContentList] Completed');
  }
...
private editFlashcard(card: Flashcard) {
  ...
  private toggleSelectAll() {
    if (this.viewType === 'cards') {
      ...
      // 如果是主界面且有选中文件，只全选该文件的内容
      const filteredUnits = (this.displayMode === 'main' && this.selectedFile) 
        ? units.filter(unit => {
            if (this.groupMode === 'file') {
              return unit.source.file === this.selectedFile;
            } else if (this.groupMode === 'tag') {
      ...
      if (this.selectedUnitIds.size === filteredUnits.length && filteredUnits.length > 0) {
        // 取消
        this.selectedUnitIds.clear();
      } else {
        // 全选当前页面的内容
